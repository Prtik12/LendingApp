...
use anchor_spl::{associated_token::AssociatedToken, token, token_interface::{self, Mint, TokenAccount, TokenInterface, TransferChecked}};
...
pub fn process_liquidate(ctx: Context<Liquidate>) -> Result<()> {
    ...
    let usdc_price = price_update.get_price_no_older_than(&Clock::get()?, MAX_AGE, &sol_feed_id)?;
